...
export class MapaPage {
  ...
  */
  ...
  private addActividadesMapa(map) {
    this.ejemploDao.getActividadesEventoByIdEvento(this.idEvento).then(arrActividades => {
      ...
      for(var i=0; i<arrActividades.length; i++) {
        ...
        /*
        var objCoord = <any>{};
        objCoord.lat = parseFloat(actividad.latitud);
        objCoord.lng = parseFloat(actividad.longitud);
        */
  ...
  /*
  private addInfoWindowActividad(actividad: ActividadEvento, map, formaGoogle: any) {
    var infoWindow = this.crearInfoWindowActividad(actividad);
    var objCoord = new Coordenada(actividad.latitud, actividad.longitud);
    var marker = new google.maps.Marker({
      position: objCoord,
      map: map,
      opacity: 0
